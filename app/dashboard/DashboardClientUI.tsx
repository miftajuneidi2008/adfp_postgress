...
export default function DashboardClientUI({data}:any) {
  ...
  const getDashboardTitle = () => {
    if (user?.role === "branch_user") return "My Submitted Applications";
    if (user?.role === "head_office_approver") return "Applications Dashboard";
    return "Dashboard";
  };
          ...
          <Card>
            <CardContent className="pt-6">
              <p className="text-sm text-slate-600 font-medium mb-1">
                {user?.role === "head_office_approver"
                  ? "New Applications"
                  : "Pending"}
              </p>
        ...
        {user?.role === "head_office_approver" && (
          <div className="bg-teal-50 border border-teal-200 rounded-lg p-4 mb-6">
            <p className="text-sm text-teal-800">
              <span className="font-semibold">Head Office Approver</span> -
              Showing all applications filtered by your assigned districts,
